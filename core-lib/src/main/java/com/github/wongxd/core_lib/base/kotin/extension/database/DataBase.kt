package com.github.wongxd.core_lib.base.kotin.extension.database
 ...
 * openhelper
 */
open class DatabaseOpenHelper private constructor(ctx: Context, dbName: String, dbVersion: Int)
    : ManagedSQLiteOpenHelper(ctx, dbName, null, dbVersion) {


    /**
     * @param columns   TuTable.ID to INTEGER + PRIMARY_KEY + UNIQUE,
     * TuTable.NAME to TEXT,
     * TuTable.ADDRESS to TEXT
     */
    constructor(ctx: Context = CoreApp.instance) : this(ctx, DB_NAME, DB_VERSION)


    private lateinit var columns: Array<Pair<String, SqlType>>

    companion object {
        //由于用户本地可能还有收藏，这里就手动输入应用包名
        val DB_NAME = "com.wongxd.absolutedomain" + ".db"
        val DB_VERSION = 3
    }
