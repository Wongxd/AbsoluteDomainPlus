package com.wongxd.absolutedomain.util.cache;
...
public class GlideCatchUtil {
    ...
    public String getCacheSize() {
        try {
            return getFormatSize(getFolderSize(Glide.getPhotoCacheDir(App.Companion.getInstance())));
        } catch (Exception e) {
            e.printStackTrace();
            return "获取失败";
        }
    ...
    public boolean cleanCatchDisk() {
        return deleteFolderFile(Glide.getPhotoCacheDir(App.Companion.getInstance()).getAbsolutePath(), true);
    }
        ...
        try {
            if (Looper.myLooper() == Looper.getMainLooper()) {
                new Thread(new Runnable() {
                    @Override
                    public void run() {
                        Glide.get(App.Companion.getInstance()).clearDiskCache();
                    }
                }).start();
            } else {
                Glide.get(App.Companion.getInstance()).clearDiskCache();
